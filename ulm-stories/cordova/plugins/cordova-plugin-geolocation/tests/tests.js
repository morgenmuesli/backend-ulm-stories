...
exports.defineAutoTests = function() {
  ...
  var isWindowsStore =
    ...
    (cordova.platformId === "windows" && !WinJS.Utilities.isPhone); // eslint-disable-line no-undef
  ...
  var skipAndroid =
    ...
    (device.isVirtual || majorDeviceVersion >= 6); // eslint-disable-line no-undef
  ...
  describe("Geolocation (navigator.geolocation)", function() {
    ...
    it("geolocation.spec.3 should contain a watchPosition function", function() {
      ...
      expect(typeof navigator.geolocation.watchPosition === "function").toBe(
        true
      );
    });
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          function(err) {
            if (err.message && err.message.indexOf("kCLErrorDomain") > -1) {
              console.log(
                "Error: Location not set in simulator, tests will fail."
              );
              expect(true).toBe(true);
              isIOSSim = true;
              done();
...
exports.defineManualTests = function(contentEl, createActionButton) {
  ...
  function setLocationDetails(p) {
    ...
    document.getElementById("altitude_accuracy").innerHTML =
