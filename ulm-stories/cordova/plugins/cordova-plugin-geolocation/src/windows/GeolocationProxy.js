...
function ensureAndCreateLocator() {
  ...
  if (
    typeof Windows.Devices.Geolocation.Geolocator.requestAccessAsync ===
    "function"
  ) {
    deferral = Windows.Devices.Geolocation.Geolocator.requestAccessAsync().then(
      function(result) {
        if (
          result === Windows.Devices.Geolocation.GeolocationAccessStatus.allowed
        ) {
          return loc;
        }
    ...
    ensureAndCreateLocator().done(function(loc) {
      if (loc) {
        ...
        fail({
          ...
          message:
            "You do not have the required location services present on your system."
        });
      }
    }, fail);
  },
