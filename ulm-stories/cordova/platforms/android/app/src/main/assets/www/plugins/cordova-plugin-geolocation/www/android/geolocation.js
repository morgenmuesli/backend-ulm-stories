cordova.define("cordova-plugin-geolocation.geolocation", function(
  require,
  exports,
  module
) {
    ...
    getCurrentPosition: function(success, error, args) {
      var win = function() {
        var geo = cordova
          ...
          .getOriginalSymbol(window, "navigator.geolocation"); // eslint-disable-line no-undef
        geo.getCurrentPosition(success, error, args);
      };
      var fail = function() {
        if (error) {
          error(
            new PositionError(PositionError.PERMISSION_DENIED, "Illegal Access")
          );
        }
      };
    ...
    watchPosition: function(success, error, args) {
      ...
      var win = function() {
        var geo = cordova
          .require("cordova/modulemapper")
          .getOriginalSymbol(window, "navigator.geolocation"); // eslint-disable-line no-undef
        pluginToNativeWatchMap[pluginWatchId] = geo.watchPosition(
          success,
          error,
          args
        );
